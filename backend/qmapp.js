class QuineMcCluskey {
  ...
  minimize() {
    ...
    const result = {
      ...
      essentialPrimeImplicants: this.formatImplicants(
        this.essentialPrimeImplicants,
      ),
      minimizedExpression: this.formatMinimizedExpression(
        this.essentialPrimeImplicants,
      ),
    };
  ...
  extractEssentialPrimeImplicants() {
    ...
    while (remainingColumns.length > 0) {
      ...
      for (let mintermIndex of remainingColumns) {
        ...
        if (countX === 1) {
          ...
          remainingRows = remainingRows.filter(
            (piIndex) => piIndex !== essentialPIIndex,
          );
          remainingColumns = remainingColumns.filter(
            (colIndex) => chart[essentialPIIndex][colIndex] !== true,
          );
          foundEssential = true;
          break;
        }
      ...
      if (!foundEssential) {
        for (let piIndex of remainingRows) {
          let coversAll = remainingColumns.every(
            (colIndex) => chart[piIndex][colIndex] === true,
          );
          if (coversAll) {
